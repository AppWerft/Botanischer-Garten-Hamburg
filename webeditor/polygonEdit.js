...
if ( typeof (google.maps.Polygon.prototype.runEdit) === "undefined") {
	...
	google.maps.Polygon.prototype.runEdit = function(flag) {
		...
		var self = this;
		if (flag) {
			var imgGhostVertex = new google.maps.MarkerImage('css/ghostVertex.png', new google.maps.Size(11, 11), new google.maps.Point(0, 0), new google.maps.Point(6, 6));
			var imgGhostVertexOver = new google.maps.MarkerImage('css/ghostVertexOver.png', new google.maps.Size(11, 11), new google.maps.Point(0, 0), new google.maps.Point(6, 6));
			...
			var createGhostMarkerVertex = function(point) {
				if (point.marker.inex < self.getPath().getLength() - 1) {
					var markerGhostVertex = new google.maps.Marker({
						position : ( typeof (google.maps.geometry) === "undefined") ? new google.maps.LatLng(point.lat() + 0.5 * (self.getPath().getAt(point.marker.inex + 1).lat() - point.lat()), point.lng() + 0.5 * (self.getPath().getAt(point.marker.inex + 1).lng() - point.lng())) : google.maps.geometry.spherical.interpolate(point, self.getPath().getAt(point.marker.inex + 1), 0.5),
						map : self.getMap(),
						icon : imgGhostVertex,
						draggable : true,
						raiseOnDrag : false
					...
					if (point.marker.inex === self.getPath().getLength() - 1) {
						var markerGhostVertex = new google.maps.Marker({
							position : ( typeof (google.maps.geometry) === "undefined") ? new google.maps.LatLng(point.lat() + 0.5 * (self.getPath().getAt(0).lat() - point.lat()), point.lng() + 0.5 * (self.getPath().getAt(0).lng() - point.lng())) : google.maps.geometry.spherical.interpolate(point, self.getPath().getAt(0), 0.5),
							map : self.getMap(),
							icon : imgGhostVertex,
							draggable : true,
							raiseOnDrag : false
		...
		var imgVertex = new google.maps.MarkerImage('css/vertex.png', new google.maps.Size(11, 11), new google.maps.Point(0, 0), new google.maps.Point(6, 6));
		var imgVertexOver = new google.maps.MarkerImage('css/vertexOver.png', new google.maps.Size(11, 11), new google.maps.Point(0, 0), new google.maps.Point(6, 6));
